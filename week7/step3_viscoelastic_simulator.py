...
def maxwell_creep(t, E1, eta1):
    return (SIGMA_0 / E1 + (SIGMA_0 / eta1) * t) * 100
...
def kv_creep(t, E2, eta2):
    tau_c = eta2 / E2
    return ((SIGMA_0 / E2) * (1 - np.exp(-t / tau_c))) * 100
...
def burgers_creep(t, E1, E2, eta1, eta2):
    ...
    return (instant + delayed + viscous) * 100
...
def update_info(model, E1, E2, eta1, eta2):
    ...
    info_text.set_text(
        ...
        f"$\\tau_2$(KV) = {tau2:.1f} s\n"
        f"즉시탄성 = {(SIGMA_0/E1)*100:.2f} %\n"
        f"점성기울기 = {(SIGMA_0/eta1)*100:.4f} %/s"
    )
